...
@login_required
def lotw_qth_locators_api(request):
    """
    API endpoint для получения данных QTH локаторов с учетом фильтров LoTW
    Локаторы берутся только первые 4 знака
    """
    if request.method != 'GET':
        return JsonResponse({'error': 'Метод не поддерживается'}, status=405)
    ...
    try:
        ...
        # Получаем уникальные пары my_callsign + локатор (первые 4 знака) + callsign
        # Используем annotate для получения первых 4 символов локатора
        ...
        for my_call, locators_dict in callsign_data.items():
            locators_list = []
            for locator, callsigns in locators_dict.items():
                locators_list.append({
                    'code': locator,
                    'callsigns': sorted(list(callsigns))
                })
            # Сортируем по коду локатора
            locators_list.sort(key=lambda x: x['code'])

            ratings.append({
                'callsign': my_call,
                'count': len(locators_list),
        ...
        # Подсчитываем общее количество локаторов
        total_locators = sum(item['count'] for item in ratings)
...
@login_required
def lotw_dxcc_api(request):
    """
    API endpoint для получения данных стран DXCC с учетом фильтров LoTW
    """
    if request.method != 'GET':
        return JsonResponse({'error': 'Метод не поддерживается'}, status=405)
    ...
    try:
        ...
        # Получаем уникальные пары my_callsign + страна DXCC + callsign
        qso_filtered = lotw_qso.filter(
            dxcc__isnull=False
        ...
        for item in qso_filtered:
            my_call = item['my_callsign']
            dxcc_country = item['dxcc']
            call = item['callsign']
            callsign_data[my_call][dxcc_country].add(call)

        # Формируем список с позывным, количеством и данными стран
        ratings = []
        for my_call, dxcc_dict in callsign_data.items():
            dxcc_list = []
            for dxcc_country, callsigns in dxcc_dict.items():
                dxcc_list.append({
                    'code': dxcc_country,
                    'callsigns': sorted(list(callsigns))
                })
            # Сортируем по названию страны
            dxcc_list.sort(key=lambda x: x['code'])
        ...
        # Подсчитываем общее количество стран
        total_countries = sum(item['count'] for item in ratings)
...
@login_required
def lotw_r150s_api(request):
    """
    API endpoint для получения данных стран Р-150-С с учетом фильтров LoTW
    """
    if request.method != 'GET':
        return JsonResponse({'error': 'Метод не поддерживается'}, status=405)
    ...
    try:
        ...
        # Получаем уникальные пары my_callsign + страна Р-150-С + callsign
        qso_filtered = lotw_qso.filter(
            r150s__isnull=False
        ).exclude(r150s='').values('my_callsign', 'r150s', 'callsign').distinct()

        # Группируем по my_callsign, затем по стране Р-150-С
        callsign_data = defaultdict(lambda: defaultdict(set))

        for item in qso_filtered:
            my_call = item['my_callsign']
            r150s_country = item['r150s']
            call = item['callsign']
            callsign_data[my_call][r150s_country].add(call)

        # Формируем список с позывным, количеством и данными стран
        ratings = []
        for my_call, r150s_dict in callsign_data.items():
            r150s_list = []
            for r150s_country, callsigns in r150s_dict.items():
                r150s_list.append({
                    'code': r150s_country,
                    'callsigns': sorted(list(callsigns))
                })
            # Сортируем по названию страны
            r150s_list.sort(key=lambda x: x['code'])

            ratings.append({
                'callsign': my_call,
                'count': len(r150s_list),
                'countries': r150s_list
            })
