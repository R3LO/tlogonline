...
from rest_api_client.database import LocalDatabase
...
class MainWindow(QMainWindow):
    ...
    def create_menu(self):
        ...
        sync_action = QAction("Синхронизировать QSO", self)
        sync_action.setShortcut("F5")
        sync_action.triggered.connect(self.sync_qsos)
    ...
    def load_qsos(self):
        """Загружает список всех QSO из локальной базы"""
        self.status_bar.showMessage("Загрузка QSO...")
        qsos = self.db.get_qsos()
        self.populate_qso_table(self.qsos_table, qsos)
        self.status_bar.showMessage(f"Загружено {len(qsos)} QSO (локально)", 5000)

    def sync_qsos(self):
        """Синхронизирует QSO с сервером"""
        self.status_bar.showMessage("Синхронизация QSO...")
        self.current_worker = WorkerThread(self.api_client.get_qsos)
        self.current_worker.finished.connect(self.on_qsos_synced)
        self.current_worker.error.connect(self.on_api_error)
        self.current_worker.start()

    def on_qsos_synced(self, qsos):
        """Обрабатывает синхронизированные QSO"""
        new_count = self.db.save_qsos(qsos)
        total_count = self.db.get_qso_count()
        self.status_bar.showMessage(f"Синхронизировано: +{new_count}, всего {total_count} QSO", 5000)
    ...
    def load_lotw_qsos(self):
        """Загружает список QSO подтверждённых через LoTW из локальной базы"""
        self.status_bar.showMessage("Загрузка LoTW QSO...")
        qsos = self.db.get_lotw_qsos()
        self.populate_lotw_table(qsos)
        self.lotw_count_label.setText(f"Всего: {len(qsos)}")
        self.status_bar.showMessage(f"Загружено {len(qsos)} LoTW QSO (локально)", 5000)

    def search_lotw_by_callsign(self):
        """Ищет QSO LoTW по позывному в локальной базе"""
        callsign = self.lotw_search_input.text().strip()
        if not callsign:
        ...
        # Ищем по позывному и фильтруем только LoTW
        results = self.db.search_by_callsign(callsign)
        ...
        self.status_bar.showMessage(f"Найдено {len(lotw_results)} LoTW QSO (локально)", 5000)
    ...
    def edit_qso(self):
        ...
        # Загружаем данные QSO из локальной базы
        self.status_bar.showMessage("Загрузка QSO...")
        qso_data = self.db.get_qso_by_id(qso_id)
        if qso_data:
            self.on_qso_loaded_for_edit(qso_data)
        else:
            QMessageBox.warning(self, "Внимание", "QSO не найден в локальной базе")
    ...
    def perform_search(self):
        """Выполняет поиск в локальной базе"""
        ...
        if search_type == 0:  # По позывному
            results = self.db.search_by_callsign(search_query)
        elif search_type == 1:  # По QTH локатору
            results = self.db.search_by_grid(search_query)
        else:  # По диапазону
            results = self.db.search_by_band(search_query)
    ...
    def on_search_results(self, results):
        ...
        self.status_bar.showMessage(f"Найдено {len(results)} записей (локально)", 5000)
